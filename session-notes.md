...
- Bug reportado pelo usuário: carrossel de fotos não passava de slide no mobile. Causa: `.gallery-arrow{display:none}` abaixo de 640px e nenhum suporte a swipe por toque. Corrigido com `src/hooks/useSwipe.ts` (novo, hook genérico de detecção de swipe horizontal) aplicado em `Carousel.tsx` (galeria do topo) e `Reviews.tsx` (carrossel de avaliações), + `touch-action:pan-y` no CSS dos dois pra não brigar com o scroll vertical da página.
**Decisões:**
